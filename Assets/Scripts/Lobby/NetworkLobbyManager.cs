...
public class NetworkLobbyManager : NetworkManager {
	...
	public int maxPlayers {
		...
		set {
			m_MaxPlayers = Mathf.Clamp (value, 1, 254);
			// 我们扩大了最大的玩家数量
			if (m_MaxPlayers > lobbySlots.Length) {
				NetworkLobbyPlayer[] slots = new NetworkLobbyPlayer[m_MaxPlayers];
				for (int i = 0; i < lobbySlots.Length; i++) {
					slots [i] = lobbySlots [i];
				}
				lobbySlots = slots;		// 更新数组
			} 
		}
	}
	public int maxPlayersPerConnection   { get { return m_MaxPlayersPerConnection; } set { m_MaxPlayersPerConnection = Mathf.Clamp (value, 1, 254); } }
	public int minPlayers                { get { return m_MinPlayers; } set { m_MinPlayers = Mathf.Clamp (value, 0, 254); } }
	...
	void SceneLoadedForPlayer(NetworkConnection conn, GameObject lobbyPlayerGameObject) {
		...
		if (LogFilter.logDebug) {
			Debug.Log ("NetworkLobby SceneLoadedForPlayer scene:" + loadedSceneName + " " + conn);
		}
	...
	public void TryToAddPlayer() {
		if (NetworkClient.active) {
			...
			if (LogFilter.logDebug) {
				Debug.Log ("NetworkLobbyManager TryToAddPlayer controllerId " + controllerId + " ready:" + ClientScene.ready);
			}
...
public static class NetworkConnectionExtensionMethods {
	...
	public static bool GetPlayerController (this NetworkConnection value, short playerControllerId, out PlayerController playerController){
